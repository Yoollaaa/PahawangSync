...
export default function Dashboard() {
  ...
  // 1. DASHBOARD SEKARANG MENGINGAT TAGIHAN DARI LOCAL STORAGE
  ...
  // 2. SELALU SIMPAN PERUBAHAN KERANJANG KE MEMORI
  useEffect(() => {
    if (cart.length > 0) {
  ...
  // 3. AUTO-BUKA MODAL PEMBAYARAN JIKA DIARAHKAN DARI HALAMAN CHECKOUT
  useEffect(() => {
    if (location.state?.openPayment) {
      setIsPaymentModalOpen(true);
      // Hapus state agar modal tidak terus terbuka saat di-refresh
      ...
      <nav className="fixed top-0 w-full z-40 bg-[#F4F8FB]/80 backdrop-blur-xl border-b border-[#E2E8F0] shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={handleLogout} className="text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-[#EF4444] transition-colors">Logout</button>
