...
import bgLaut from '../assets/background.jpg';
    ...
    const fetchProducts = async () => {
      try {
        ...
        const formattedData = response.data.map(item => ({
          ...item,
          image: item.image_url || "https://images.unsplash.com/photo-1499793983690-e29da59ef1c2?q=80&w=1000&auto=format&fit=crop", 
          description: item.description || "Fasilitas berkualitas dan pelayanan khusus di Pulau Pahawang."
        }));
...
return (
    <div 
      className="min-h-screen font-sans pb-24 selection:bg-[#0EA5E9] selection:text-white bg-cover bg-center bg-fixed relative"
      style={{ backgroundImage: `url(${bgLaut})` }}
    >
      ...
      <div className="relative z-10 text-[#0F172A]">
        ...
        <nav className="fixed top-0 w-full z-40 bg-white/70 backdrop-blur-xl border-b border-white shadow-sm">
          <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
            ...
            <div className="flex items-center gap-6">
              <button onClick={() => setIsCartOpen(true)} className="relative p-2 text-slate-600 hover:text-[#0284C7] transition-colors cursor-pointer active:scale-95">
                <IconCart />
              ...
              <button onClick={handleLogout} className="text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-red-500 transition-colors">Logout</button>
        ...
        <main className="max-w-7xl mx-auto px-6 pt-32 relative z-10">
          <header className="mb-12">
            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-white mb-3 drop-shadow-lg">
              Eksplorasi laut, <br/>
              <span className="text-white font-serif italic font-medium drop-shadow-lg">{user.name}.</span>
            </h2>
            <p className="text-slate-700 font-medium bg-white/80 inline-block px-4 py-1.5 rounded-full backdrop-blur-md border border-white shadow-sm">
              Temukan keindahan tersembunyi di Pulau Pahawang hari ini.
            </p>
          </header>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-16">
            
            <div onClick={() => setIsPaymentModalOpen(true)} className="md:col-span-2 group bg-[#0284C7]/80 backdrop-blur-md border border-white/20 rounded-[24px] p-8 text-white relative overflow-hidden cursor-pointer hover:shadow-xl hover:shadow-[#0284C7]/30 hover:-translate-y-1 transition-all duration-300">
               <div className="absolute right-0 bottom-0 opacity-30 pointer-events-none group-hover:scale-110 transition-transform duration-700">
                <svg width="200" height="200" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M200 200H0V100C40 100 60 140 100 140C140 140 160 100 200 100V200Z" fill="#38BDF8"/>
                  <path d="M200 200H0V150C30 150 50 180 100 180C150 180 170 150 200 150V200Z" fill="#E0F2FE"/>
                </svg>
              </div>
              <div className="relative z-10 flex flex-col h-full justify-between min-h-[160px]">
                <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/30 mb-4 shadow-inner">
                  <IconCart />
                </div>
                <div>
                  <h3 className="text-2xl font-bold mb-1 drop-shadow-sm">Menu Pembayaran</h3>
                  <p className="text-[#E0F2FE] text-sm font-medium drop-shadow-sm">Cek status tagihan dan selesaikan pesananmu.</p>
                </div>
              </div>
            </div>
                        <div onClick={handleOpenTicketModal} className="group bg-white/85 backdrop-blur-md border border-white rounded-[24px] p-8 text-[#0F172A] cursor-pointer hover:bg-white hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
              <div className="flex flex-col h-full justify-between min-h-[160px]">
                ...
                <div>
                  <h3 className="text-xl font-bold mb-1">Tiket Digital</h3>
                  <p className="text-slate-500 text-sm font-medium">Akses QR Code pesanan.</p>
                </div>
              </div>
            </div>

            <div className="group bg-white/85 backdrop-blur-md border border-white rounded-[24px] p-8 text-[#0F172A] cursor-pointer hover:bg-white hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
               <div className="flex flex-col h-full justify-between min-h-[160px]">
                ...
                <div>
                  <h3 className="text-xl font-bold mb-1">Profil Akun</h3>
          ...
          <section>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 bg-white/85 backdrop-blur-md p-4 rounded-[20px] border border-white shadow-sm">
              ...
              <div className="flex gap-2 overflow-x-auto hide-scrollbar w-full md:w-auto">
                {categories.map((cat) => (
                  <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-6 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 whitespace-nowrap ${activeCategory === cat ? 'bg-[#0284C7] text-white shadow-md' : 'bg-transparent text-slate-600 hover:bg-white hover:text-[#0284C7]'}`}>{cat}</button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProducts.length > 0 ? (
                filteredProducts.map(item => <ProductCard key={item.id} title={item.name} category={item.category} price={item.price} image={item.image} description={item.description} onAddToCart={() => setSelectedProduct(item)} />)
              ) : (
                <div className="col-span-full py-24 flex flex-col items-center justify-center bg-white/85 backdrop-blur-md rounded-[24px] border border-white shadow-sm">
        ...
        {isPaymentModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0F172A]/60 backdrop-blur-sm p-4 text-slate-800">
        ...
        {selectedProduct && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 bg-[#0F172A]/40 backdrop-blur-sm transition-all text-slate-800">
        ...
        <div className={`fixed top-0 right-0 h-full w-full md:w-[450px] bg-white z-[80] shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col text-slate-800 ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
